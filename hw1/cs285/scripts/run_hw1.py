...
def run_training_loop(params):
    ...
    # Set random seeds -> program could have same result
    seed = params['seed']
    np.random.seed(seed)
    ...
    for itr in range(params['n_iter']):
        ...
        for _ in range(params['num_agent_train_steps_per_iter']):
            ...
            perm = np.random.permutation(replay_buffer.obs.shape[0])[:params['train_batch_size']]
            #   print(sample_list)
            ob_batch = replay_buffer.obs[perm]
            ac_batch = replay_buffer.acs[perm] 
